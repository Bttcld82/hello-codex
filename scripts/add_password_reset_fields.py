    ...
    with app.app_context():
        ...
        with db.engine.connect() as conn:
            if 'reset_token' not in columns:
                conn.execute(db.text("ALTER TABLE people ADD COLUMN reset_token VARCHAR(100)"))
                print("Added reset_token column")
            
            if 'reset_token_expiry' not in columns:
                conn.execute(db.text("ALTER TABLE people ADD COLUMN reset_token_expiry DATETIME"))
